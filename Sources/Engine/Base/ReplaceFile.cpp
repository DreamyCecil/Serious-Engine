...
BOOL GetReplacingFile(CTFileName fnSourceFile, CTFileName &fnReplacingFile, const char *pFilter)
{
  ...
  try
  {
    ...
    strNewRemap.PrintF("\"%s\" \"%s\"\n", fnSourceFile.ConstData(), fnReplacingFile.ConstData());
    strBase += strNewRemap;
    ...
    catch( char *strError)
    {
      ...
      else
      {
        ...
        if (wed_bUseGenericTextureReplacement) {
          ...
          ThrowF_t(TRANS("Unable to load world because texture \"%s\" can't be found."), fnmTexture.ConstData());
        }
      }
    }
  }
  ...
  for(;;) {
    try {
      ...
      if( GetReplacingFile( fnTexture, fnReplacingTexture, FILTER_TEX FILTER_END)) {
        ...
        ThrowF_t(TRANS("Cannot find substitution for \"%s\""), fnTexture.ConstData());
      }
    }
  }
  ...
  for(;;) {
    try {
      ...
      if( GetReplacingFile( fnModel, fnReplacingModel, FILTER_MDL FILTER_END)) {
        ...
        ThrowF_t(TRANS("Cannot find substitution for \"%s\""), fnModel.ConstData());
      }
    }
  }
...
void WriteAnimQueue_t(CTStream &strm, CModelInstance &mi)
{
  ...
  for(INDEX ial=0;ial<ctal;ial++) {
    ...
    strm<<(FLOAT)al.al_tmStartTime;
    strm<<(FLOAT)al.al_tmFadeTime;
...
void ReadModelInstanceOld_t(CTStream &strm, CModelInstance &mi)
{
  ...
  for(INDEX ial=0;ial<ctal;ial++) {
    ...
    strm >> fDummy;
    al.al_tmStartTime = (DOUBLE)fDummy;
    strm >> fDummy;
    al.al_tmFadeTime = (DOUBLE)fDummy;
    ...
    for(INDEX ipa=0;ipa<ctpa;ipa++) {
      ...
      strm >> fDummy;
      pa.pa_tmStartTime = (DOUBLE)fDummy;
  ...
  for(INDEX ial=0;ial<ctal;ial++) {
    ...
    for(INDEX ipa=0;ipa<ctpa;ipa++) {
      ...
      if(strm.PeekID_t()==CChunkID("PASP")) {
        ...
        strm >> fDummy;
        pa.pa_tmSpeedMul = (DOUBLE)fDummy;
      }
    }
  }
}
  ...
  for(;;) {
    try {
      ...
      if( GetReplacingFile( fnAnim, fnReplacingAnim, FILTER_ANI FILTER_END)) {
        ...
        ThrowF_t(TRANS("Cannot find substitution for \"%s\""), fnAnim.ConstData());
      }
    }
  }
  // read anim object
