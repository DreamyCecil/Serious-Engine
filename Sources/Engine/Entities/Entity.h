...
public:
  ...
  SECOND GetSoundLength(SLONG idSoundComponent);
...
public:
  ...
  // [Cecil] NOTE: Returns TRUE if this is a prediction of the original entity (i.e. this is a "predictor" for some entity)
  inline BOOL IsPredictor(void) const { return (en_ulFlags & ENF_PREDICTOR) != 0; };

  // [Cecil] NOTE: Returns TRUE if this is the original entity (i.e. this entity is currently "predicted" using its copy)
  inline BOOL IsPredicted(void) const { return (en_ulFlags & ENF_PREDICTED) != 0; };

  // [Cecil] NOTE: Returns TRUE if this entity can create predictions of itself
  inline BOOL IsPredictable(void) const { return (en_ulFlags & ENF_PREDICTABLE) != 0; };

  // [Cecil] NOTE: Returns prediction of this original entity (i.e. "predictor" copy of this entity)
  CEntity *GetPredictor(void);

  // [Cecil] NOTE: Returns original entity of this prediction (i.e. entity that's being "predicted" using this one)
  ...
  // [Cecil] NOTE: Returns a pointer to either one:
  // - Prediction of this entity, if it's the original one
  // - Itself, if it's already a prediction of some entity (safe to call recursively)
  // - Itself, if this entity cannot be predicted (in this case the use of this function is discouraged)
  CEntity *GetPredictionTail(void);

  // [Cecil] NOTE: Returns TRUE if network prediction is enabled and this is the original entity
  ...
  // [Cecil] NOTE: Returns entity from m_penPrediction property (autogenerated by ECC)
  // If it's an original entity, returns a pointer to the current prediction copy
  // If it's a prediction copy, returns a pointer to the original entity
  virtual CEntity *GetPredictionPair(void);

  // [Cecil] NOTE: Interconnects entity with its prediction via m_penPrediction property (autogenerated by ECC)
