...
void SE_ParseCommandLine(const CommandLineSetup &cmd) {
  ...
  for (INDEX i = 0; i < ct; i++) {
    ...
    if (i == 0 && cmd.pInitialArgs.pFunc != NULL) {
      ...
      CommandLineSetup::Commands_t::const_iterator itCmd = cmd.mapCommands.find(strCmd);

      // Command not found
      if (itCmd == cmd.mapCommands.end()) {
        // Handle unknown commands
        BOOL bHandled = FALSE;

        if (cmd.pUnknownOption != NULL) {
          bHandled = cmd.pUnknownOption(strCmd);
        }

        // Not handled
        if (!bHandled) {
          _strCmdOutput += CTString(0, TRANS("  Unknown option: '%s'\n"), strCmd);
        }
    ...
    if (ctArgs > ctLeft) {
      ...
      _strCmdOutput += CTString(0, TRANS("  '%s' requires %d arguments!\n"), strCmd, ctArgs);
