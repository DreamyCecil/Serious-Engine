...
Rect _rectInvalid;
TUpdateable<true> _udAutoGenerateDistribution;
TChangeable<true> _chAutoGenerateDistribution;
...
{
  pToolTipText += sprintf(pToolTipText, "%s", _strToolTip.ConstData());
}
...
void ToggleFlag(CTIButton *ptib, CPoint pt, CDrawPort *pdp)
{
  ...
  if( iLayer!=-1)
  {
    ...
    if(ptlLayer!=NULL)
    {
      BOOL bOldValue= *(BOOL *)((UBYTE *)ptlLayer + (UINT_PTR)ptib->tib_pfData1);
      bOldValue = !bOldValue;
      *(BOOL *)((UBYTE *)ptlLayer + (UINT_PTR)ptib->tib_pfData1) = bOldValue;
    }
  }
}
