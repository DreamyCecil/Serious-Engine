...
See the list of the most notable fork features here: https://github.com/DreamyCecil/Serious-Engine/wiki/Fork-features
...
Before building, make sure that you have all the necessary submodules in the repository by running this command:
```
...
```
git clone --recurse-submodules -j8 https://github.com/DreamyCecil/Serious-Engine.git
```
...
1. Install **Git** and **CMake** tools:
```
sudo apt install git cmake
```
2. Install **GCC** and other build tools (if you don't already have GCC):
```
sudo apt install build-essential
```
3. Install **Bison** and **Flex** tools (if you've installed GCC separately without them):
```
sudo apt install bison flex
```
4. Install **OpenAL** library:
```
sudo apt-get install libopenal-dev
```
5. Clone the repository in any directory with all submodules:
```
git clone --recurse-submodules -j8 https://github.com/DreamyCecil/Serious-Engine.git
```
6. Update all submodules in the repository:
```
git submodule update --init --recursive
```
...
> - To prevent entity source files from rebuilding (e.g. when building from a Windows filesystem), add `-DSE1_COMPILE_ES=0` to the CMake command.
...
<details>
<summary>Simple DirectMedia Layer (SDL)</summary>
...
<details>
<summary>3D Exploration</summary>
...
<details>
<summary>The OpenGL Extension Wrangler Library (GLEW)</summary>
...
- Even though **Visual Studio 2010** can be used for building, the code for its support is filled with many awkward workarounds and it generally hasn't been tested very well. Such an old IDE should only be used in very specific cases where a newer Visual Studio won't cut it.
  - **SDL3** seemingly cannot be built using earlier VS versions (like 2010 or 2013), requiring a newer VS IDE or prebuilt binaries in the respective build directory.
